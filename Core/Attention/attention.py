...
class NaylisAttention(nn.Module):
    ...
    def __init__(
        ...
        # ── Partial RoPE (25% des dims) ──────────────────────────
        # Le GDN encode déjà la position via sa récurrence h_t.
        # Appliquer RoPE sur 100% saturerait la sémantique et créerait
        # un conflit de signal avec les portes alpha/beta du GDN.
        # 25% = 16 dims sur 64 : la position est encodée sans écraser
        # les 48 dims sémantiques restantes.
        self.rope_dim = max(2, (self.head_dim // 4) & ~1)  # 25%, toujours pair
        if use_rope:
            self.rope = RotaryPositionalEmbedding(
                self.rope_dim, max_seq_len,
                use_yarn              = use_yarn,
    ...
    def _attn_scale(self) -> float:
        # Le scale d'attention porte toujours sur head_dim complet (64 dims).
        # Le facteur YaRN s'applique sur la fréquence des dims RoPE (rope_dim),
        # mais le denominateur du softmax reste sqrt(head_dim) — pas sqrt(rope_dim).
    ...
    def forward(
        ...
        if self.use_rope and self.rope is not None:
            # Partial RoPE : rotation sur les rope_dim premières dims seulement
            # Les dims restantes (sémantiques) ne sont pas touchées
            q_r = q[..., :self.rope_dim]
            k_r = k[..., :self.rope_dim]
            q_r, k_r = self.rope(q_r, k_r, position_offset=position_offset)
        ...
        if use_varlen:
            ...
            # Cast fp32 systématique AVANT tout ajout de biais et masquage :
            # - graph_bias asymétrique → erreurs d'arrondi bf16 au softmax
            # - masked_fill avec float('-inf') en bf16 peut produire des NaNs
            # Ce cast unique couvre les deux cas (graph_bias présent ou non).
            scores = scores.float()
            if graph_bias is not None:
                scores = scores + graph_bias.float()
